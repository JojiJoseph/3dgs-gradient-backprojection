...
def render_to_gif_blender(
    ...
    for frame in tqdm(splats["transforms"]["frames"]):
        ...
        viewmat[:3, :3] = viewmat[:3, :3] @ torch.tensor(([1, 0, 0], [0, -1, 0], [0, 0, -1])).float()
...
def render_mask_2d_to_gif_blender(
    splats,
    features,
    prompt,
    neg_prompt,
...
def main(
    ...
    render_to_gif_blender(
        f"{results_dir}/extracted.gif",
        extracted,
        show_visual_feedback,
        use_checkerboard_background=True,
    )
    render_to_gif_blender(f"{results_dir}/deleted.gif", deleted, show_visual_feedback)
