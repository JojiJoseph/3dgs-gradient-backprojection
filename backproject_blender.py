...
def create_feature_field_lseg_blender(splats, batch_count = 0, use_cpu=False):
    ...
    # gaussian_features = torch.zeros(colors.shape[0], 512, device=colors.device)
    # gaussian_denoms = torch.ones(colors.shape[0], device=colors.device) * 1e-12
    # writer = cv2.VideoWriter("output_bp.avi", cv2.VideoWriter_fourcc(*"MJPG"), 10, (2592, 840))
    # for batch_start in tqdm(
    #         range(0, len(images), batch_size),
    #         desc="Feature backprojection (batches)",
    # ):
    ...
    #             feats_pca = pca.transform(feats.reshape((-1,512)).cpu().numpy())
    #             feats_pca = feats_pca - np.min(feats_pca, axis=0)
    #             feats_pca = feats_pca / np.max(feats_pca, axis=0)
    #             # feats_pca = torch.tensor(feats_pca, device=device).float().reshape((height, width, 3))
    ...
    #         gaussian_features_temp = gaussian_features / gaussian_denoms[..., None]
    #         gaussian_features_temp = gaussian_features_temp / gaussian_features_temp.norm(dim=-1, keepdim=True)
    #         gaussian_features_temp[torch.isnan(gaussian_features_temp)] = 0

    #         features_pca = pca.transform(gaussian_features_temp.cpu().numpy())
    #         features_pca = features_pca - np.min(features_pca, axis=0)
    #         features_pca = features_pca / np.max(features_pca, axis=0)
    #         viewmat_here = torch.eye(4).float()
    #         viewmat_here[1,-1] = -1.5
    #         viewmat_here[2,-1] = 5.5
    #         viewmat_here = torch.tensor([[ 0.8613, -0.1055,  0.4970,  0.1400],
    #     [ 0.1519,  0.9869, -0.0537, -1.3700],
    #     [-0.4848,  0.1217,  0.8661,  5.5500],
    #     [ 0.0000,  0.0000,  0.0000,  1.0000]], device='cuda:0')
    #         output_pca_scene, _, _ = rasterization(
    #             means,
    #             quats,
    #             scales,
    #             opacities,
    #             torch.tensor(features_pca, device=device).float(),
    #             viewmat_here[None],
    #             K[None],
    #             width=width,
    #             height=height,
    ...
    #         output_pca_scene_cv = torch_to_cv(output_pca_scene[0])
    #         feats_pca = np.clip(feats_pca,0,1) * 255
    #         feats_pca = feats_pca.astype(np.uint8)
    #         print(feats_pca.shape, output_pca_scene_cv.shape)
    #         output_pca = cv2.hconcat([feats_pca[...,::-1], output_pca_scene_cv])
    #         cv2.imshow("PCA Scene", output_pca)
    #         writer.write(output_pca)
