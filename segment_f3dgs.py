...
def render_mask_2d_to_gif_f3dgs(
    splats,
    prompt,
    neg_prompt,
    output_path: str,
...
def main(
    ...
    features = splats["features"] @ splats["conv"]
    ...
    render_mask_2d_to_gif_f3dgs(
        splats,
        prompt,
        neg_prompt,
        f"{results_dir}/mask2d_f3dgs.gif",
        show_visual_feedback,
    )

    render_to_gif_f3dgs(
        f"{results_dir}/extracted_f3dgs.gif",
        extracted,
        show_visual_feedback,
        use_checkerboard_background=True,
    )
    render_to_gif_f3dgs(f"{results_dir}/deleted_f3dgs.gif", deleted, show_visual_feedback)

    if export_checkpoint:
        save_to_ckpt(f"{results_dir}/extracted_f3dgs.pt", extracted)
        save_to_ckpt(f"{results_dir}/deleted_f3dgs.pt", deleted)
