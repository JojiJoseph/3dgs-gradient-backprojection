...
import numpy as np
import matplotlib
from feature_extractors import DinoFeatureExtractor, LSegFeatureExtractor, get_feature_extractor, BACKPROJECTION_FEATURE_EXTRACTORS
...
def create_feature_field(splats, feature_type="lseg", use_cpu=False):
    ...
    gaussian_features = torch.zeros(colors.shape[0], feature_extractor.dim, device=colors.device)
    gaussian_denoms = torch.ones(colors.shape[0], device=colors.device) * 1e-12

    t1 = time.time()

    colors_feats = torch.zeros(colors.shape[0], feature_extractor.dim, device=colors.device)
...
def main(
    ...
    if feature not in BACKPROJECTION_FEATURE_EXTRACTORS:
        raise ValueError(f"Invalid feature extractor: {feature}. Available options: {', '.join(BACKPROJECTION_FEATURE_EXTRACTORS.keys())}")
    ...
    features = create_feature_field(
        splats, feature_type=feature)
    torch.save(
        features, f"{results_dir}/features_{feature}.pt")
