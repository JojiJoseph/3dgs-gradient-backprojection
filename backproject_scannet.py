...
def create_feature_field(
    ...
    elif feature_type in ["scannet-instance", "scannet-lseg"]:
        feature_dir = splats["feature_dir"]
        data_dir = splats["data_dir"]
    ...
    data_dir = splats["data_dir"]
    scannet_img_dir = os.path.join(data_dir, "output", "color")
    scannet_pose_dir = os.path.join(data_dir, "output", "pose")
    width = splats["width"]
    height = splats["height"]
    ...
    for frame in tqdm(
        get_frames_scannet(data_dir, percentage_frames=percentage_frames, n_views=n_views)
    ):
...
class Args:
    ...
    tag: str = None  # Tag to append to output filename
...
def main(
    ...
    if feature in ["feature-map", "lang-splat"]:
        ...
        splats["level"] = level
    if feature in ["scannet-instance", "scannet-lseg"]:
        splats["feature_dir"] = feature_dir
        splats["data_dir"] = data_dir
