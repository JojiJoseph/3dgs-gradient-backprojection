...
def create_feature_field(
    ...
    if "colmap_project" in splats:
        colmap_project = splats["colmap_project"]
        project = colmap_project
    else:
        transforms = splats["transforms"]
        project = transforms
    ...
    if "colmap_project" in splats:
        ...
        for frame in tqdm(
            get_frames_blender(transforms, percentage_frames=percentage_frames, n_views=n_views)
        ):

            width = int(K[0, 2] * 2)
            height = int(K[1, 2] * 2)
            viewmat = get_viewmat_from_blender_frame(frame).to(device)
            metadata = frame
            # viewmat = frame["viewmat"].to(device)
            with torch.no_grad():
                output, _, meta = rasterization(
